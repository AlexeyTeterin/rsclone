...
const onLearnMoreClick = (event: Event) => {
  ...
  // const poster = createElement('img', 'modal-poster',
  // 'rounded', 'mx-auto', 'd-block') as HTMLImageElement;
  ...
  // imdbRating.innerHTML = `<b>IMDB Rating:</b>
  // <span class="badge bg-warning text-dark modal-rating">$
  // { data.imdbRating }</span > (${ data.imdbVotes } votes)`;
  ...
  const modal = document.querySelector('#movieModal .modal-dialog')!;

  ReactDOM.unmountComponentAtNode(modal);
  ReactDOM.render(<MovieModal movie={data} />, modal);
